...
/**
 * Generate inline keyboard for sources with pagination
 */
export function generateSourcesKeyboard(
  ...
  const endIndex = Math.min(startIndex + pageSize, sources.length);
  const pageSources = sources.slice(startIndex, endIndex);

  // Create buttons for each source
  const keyboard = pageSources.map((source) => [
    createInlineButton(
      `📦 ${source.displayName}`,
      `select_source:${source.name}`
    ),
  ]);
  ...
  if (page > 0) {
    paginationRow.push(
      createInlineButton('⬅️ Anterior', `sources_page:${page - 1}`)
    );
  }

  if (page < totalPages - 1) {
    paginationRow.push(
      createInlineButton('Próxima ➡️', `sources_page:${page + 1}`)
    );
  }
  ...
  // Add close button
  keyboard.push([createInlineButton('❌ Fechar', 'close')]);

  const text = `<b>📚 Sources (${sources.length} total)</b>\n` +
